...
int farray_data_file_to_memory_from_stdin (FILE* filename, size_t size_max, char** mem_space, size_t* mem_size);
  //
  //  Allocate size_max of memory space and dump the data from the standart stdin to that memory.
  //  Return the size of the memory space and a pointer to it (mem_space, mem_size).
  //  Return code : O on success,  2 on memory error.
  //
  //  WARNING : the caller of that function has the RESPONSABILITY of the
  //            allocated memory (ie deallocation).
  //
